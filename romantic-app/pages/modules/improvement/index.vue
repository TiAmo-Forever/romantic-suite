<template>
  <view class="page app-page-shell app-page-shell-tabbed improvement-page" :style="themeStyle">
    ...
    <view class="hero-card app-fade-up">
      ...
      <view class="hero-title">把记录变成靠近彼此的小行动</view>
      ...
      >
        <view class="note-body">
          ...
          <view class="note-meta-row">
            <view class="note-date">{{ item.startDate || '未设置开始日期' }}</view>
            <view class="note-meta-sep">·</view>
            <view class="note-progress">{{ resolveElapsedLabel(item) }}</view>
          </view>

          <view class="note-tracking-row">
            <view class="tracking-chip" :class="resolveTodayTrackingClass(item)">
              <text class="tracking-chip-label">今天</text>
              <text class="tracking-chip-value">{{ resolveTodayFeedbackValue(item) }}</text>
            </view>
            <view class="tracking-chip tracking-chip-total">
              <text class="tracking-chip-label">累计</text>
              <text class="tracking-chip-value">{{ resolveTotalFeedbackValue(item) }}</text>
            </view>
          </view>

          <view class="note-desc">{{ item.latestFeedback || '还没有反馈' }}</view>
          ...
          <view v-if="item.mediaList?.length" class="media-summary">
            ...
            <view class="media-summary-text">
              ...
              <view class="media-summary-desc">点击查看</view>
            </view>
          </view>

          <view class="note-footer">
            <view class="note-tone">{{ statusToneMap[item.status] }}</view>
            <view class="note-creator">{{ item.creatorNickname ? `记录人：${item.creatorNickname}` : '未登记记录人' }}</view>
...
function resolveTodayFeedbackValue(item) {
  return `${Math.max(0, Number(item?.todayFeedbackCount || 0))} 次`
}

function resolveTotalFeedbackValue(item) {
  return `${Math.max(0, Number(item?.feedbackCount || 0))} 次`
}

function resolveTodayTrackingClass(item) {
  const today = Math.max(0, Number(item?.todayFeedbackCount || 0))
...
<style scoped>
  ...
  .hero-desc { margin-top: 14rpx; font-size: 24rpx; line-height: 1.75; color: rgba(255,255,255,0.94); }
  ...
  .note-heading { flex: 1; min-width: 0; position: relative; padding-top: 10rpx; }
  .note-status-ribbon {
    display: inline-flex;
    align-items: center;
    gap: 8rpx;
    padding: 10rpx 18rpx 10rpx 14rpx;
    border-radius: 16rpx 999rpx 999rpx 16rpx;
    font-size: 19rpx;
    font-weight: 700;
    line-height: 1;
    box-shadow: 0 10rpx 18rpx rgba(255, 159, 186, 0.16);
  }
  ...
  .note-date,
  .note-progress { line-height: 1.4; }
  .note-meta-sep { color: #d2a7b4; line-height: 1; }
  .note-tracking-row {
    margin-top: 12rpx;
    display: flex;
    align-items: center;
  ...
  .tracking-chip-active {
    background: linear-gradient(135deg, #ffe8f0, #ffd5e0);
    color: #bb527d;
  }
  .tracking-chip-total {
    background: #fff3f7;
  ...
  .note-footer { margin-top: auto; padding-top: 14rpx; display: flex; align-items: center; gap: 14rpx; }
  .note-tone { color: #bb8195; font-size: 18rpx; line-height: 1; flex-shrink: 0; }
  .note-creator { font-size: 20rpx; color: #bc8b9b; flex: 1; min-width: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
